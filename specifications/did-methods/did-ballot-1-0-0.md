# Trusted Digital Web "DID Ballot" Decentralized Identifier Method Namespace Specification version 1.5.0
...
This "DID Ballot" Decentralized Identifier Method Namespace Specification ("DID Ballot" DID Method Namespace Specification) 
...
that are compliant with the "DID Ballot" DID Method Namespace Specification.

### Out-of-Scope

The following topics are out-of-scope:
- Descriptions of any implementation-specific design, implementation, or deployment details for any particular software platform that might support the "DID Ballot" DID Method Namespace.
...
- The term "DID Ballot" Identifier is synonymous with the term "DID Ballot" Decentralized Identifier. 
- The term "DID Ballot" DID Document is synonymous with the term "DID Ballot" Decentralized Identifier Document.
- The term "DID Ballot" DID Method Namespace Specification is synonymous with the terms "DID Ballot" Decentralized Identifier Method Namespace Specification and Trusted Digital Web "DID Ballot" DID Method Namespace Specification.
- A compliant Trusted Digital Web Runtime Library is a software library that, in part, implements the "DID Ballot" DID Document CRUD Abstract Interface in a way that complies with the "DID Ballot" DID Method Namespace Specification.
...
The authoriative source text for this specifcation can be found here: https://github.com/mwherman2000/TrustedDigitalWeb/blob/master/specifications/did-methods/did-ballot-1-0-0.md.

The list of the most recent versions of this specification can be found here: https://github.com/mwherman2000/TrustedDigitalWeb/blob/master/specifications/did-methods/did-ballot.md.

## 2. Trusted Digital Web "DID Ballot" Identifier Method Name

The name string that shall identify the "DID Ballot" Identifier Method is: `ballot`.

A DID Identifier that uses this method **MUST** begin with the following prefix: `did:ballot`. 
Per the DID Decentralized Identifier specification[2], the value of this string **MUST** be in lowercase.

## 3. Trusted Digital Web "DID Ballot" Identifier Format
...
The KERI seed value can be any globally unique value including, for example, a composite database record key, ballot key, or GUID value. 
Trusted Digital Web "DID Ballot" Identifier `id-string` values are encoded using the KERI `Base64` encoding method.
...
### Example 1. Trusted Digital Web "DID Ballot" Identifier
```
did:ballot:examplesystem:exampleelection:BF5pxRJP6THrUtlDdhh07hJEDKrJxkcR9m5u1xs33bhp
```
...
"DID Ballot" Identifiers and associated DID Documents on the Trusted Digital Web are managed by a compliant Trusted Digital Web Runtime Library. 
The Library implements CRUD interfaces for controlling the lifecycle of a "DID Ballot" Identifier and its associated DID Document that are compliant with this specification.
 
### 4.1 Create (Register)

To create a "DID Ballot" Identifier, a program invokes the `RegIdWithPublicKey` function from a compliant Trusted Digital Web Runtime Library. 
The interface to register a "DID Ballot" Identifier and its associated public key is defined as follows:
```csharp
public bool RegIdWIthPublicKey(string didobjectid, byte[] publicKey); 
```
The calling program must include two parameters: the string value of the new "DID Ballot" Identifier to be registered and 
a cryptographic public key to act as the first management key. 
This function will return `True` if the "DID Ballot" Identifier had not been registered previously.

### 4.2 Read (Resolve)

Trusted Digital Web "DID Ballot" Identifier's associated DID Document can be looked up by invoking the `GetDIDDocument` function from a compliant Trusted Digital Web Runtime Library. 
...
The interface for resolving a "DID Ballot" Identifier and return its associated DID Document is defined as follows:
```csharp
public DIDDocument GetDIDDocument(string didobjectid);
```
A DIDDocument is a JSON-ballot which contains the `verificationMethod`, `authentication` elements of the associated DID Document.
Every public key in the array of `verificationMethod` can be used to authenticate the "DID Ballot" Controller.

Note: The list of supported public key signature schemes is listed in [Appendix A](#appendix-a-public-key-algorithm).

### Example 2. Trusted Digital Web "DID Ballot" DID Document
...
To update the DID Document associated with a "DID Ballot" Identifier, two functions need to be invoked, 
```csharp
...
```
Note that `sender` param must be a currently-in-use public key of this "DID Ballot" Identifier.
If a public key is removed, then it **cannot** be added again.

### 4.4 Deactivate (Revoke)

To delete or deactivate a "DID Ballot", it suffices to remove all public keys from its associated 
DID Document. In this case, there is no public key that can be used to authenticate the "DID Ballot" Controller.

More importantly, deletion of a Trusted Digital Web "DID Ballot" DID Document means that the associated "DID Ballot" Identifier cannot be reactivated again. 
...
There are no security considerations that are specific to this DID Method Namespace Specification. Security considersations and requirements are the responsiblity of the particular platform Implementor - taking into consideration the classes of "DID Ballots" processed by that platform.
...
that, in turn, leverage the "DID Ballot" Method Namespace Specification; as one example, whether the underlying VDR is configured to support concensus by PoA, PoS, or PoW 
(Reference: https://academy.stratisplatform.com/Architecture%20Reference/FullNode/Consensus/consensus-introduction.html).

More importantly, Implementers need to be highly aware of the types of Fully Decentralized Ballots they are managing and processing on the Trusted Digital Web. 
...
There are no privacy considerations that are specific to this DID Method Namespace Specification. Privacy considersations and requirements are the responsiblity of the particular platform Implementor - taking into consideration the classes of "DID Ballots" processed by that platform.

The syntax and construction of a "DID Ballot" Identifier and its associated DID Document helps to ensure that no Personally Identifiable Information (PII) or other personal data is exposed by these constructs.

Implementers need to be aware of the privacy implications of the underlying tools and technologies 
used to develop agents, services, and libraries for the Trusted Digital Web that, that in turn, leverage the "DID Ballot" Method Namespace Specification. 
...
The current version of the code for the "DID Ballot" Identifier Method reference implementation can be found in the following GitHub project: https://github.com/mwherman2000/TrustedDigitalWeb. 
This project is the definitive reference implementation of the Trusted Digital Web "DID Ballot" Identifier Method and contains compliant reference implementations of apps, agents, and services deployable on the Trusted Digital Web.
...
The Trusted Digital Web includes a Trusted Digital Web Runtime Library that is compliant with the "DID Ballot" DID Method Namespace Specification. "DID Ballot" Identifiers and the "DID Ballot" Identifer Method are two of the components and specifications that are foundational in the architecture and design of the Trusted Digital Web.
...
[5] The Verifiable Economy Architecture Reference Model (VE-ARM): Fully Decentralized Ballot (FDO) Model, https://hyperonomy.com/2021/04/26/the-verifiable-economy-architecture-reference-model-ve-arm-fdo/.

[6] The Verifiable Economy: Fully Decentralized Ballot (FDO) Example: Bob’s UDID Document, https://hyperonomy.com/2021/06/15/the-verifiable-economy-fully-decentralized-ballot-fdo-example-bobs-udid-document/.
...
[42] Electoral System, http://en.m.wikipedia.org/wiki/Electoral_system
