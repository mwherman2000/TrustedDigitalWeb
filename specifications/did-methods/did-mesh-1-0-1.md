...
The DIDComm Agent Mesh Networks Decentralized Identifier Method Namespace Specification 
("DIDMesh" DID Method Namespace Specification) 
defines the end-to-end lifecycle of DID Identifiers and DID Documents for "DIDMesh" DIDComm Agent Mesh Networks.

This specification defines the following:
- "DIDMesh" Identifier Syntax and Construction
- "DIDMesh" DID Document CRUD Abstract Interface

This specification also provides Platform Implementation Guidance to assist Implementors in creating safe and secure apps, agents, services and platforms that are compliant with the "DIDMesh" DID Method Namespace Specification.

### Out-of-Scope

The following topics are out-of-scope:
- Descriptions of any implementation-specific design, implementation, or deployment details for any particular software platform that might support the "DIDMesh" DID Method Namespace.

### Intended Audience

The primary audience for this DID Method Namespace Specification is Implementors of apps, DIDComm agents, and services; including but not limited to: 
...
- The term "DIDMesh" Identifier is synonymous with the term "DIDMesh" Decentralized Identifier. 
- The term "DIDMesh" DID Document is synonymous with the term "DIDMesh" Decentralized Identifier Document.
- The term "DIDMesh" DID Method Namespace Specification is synonymous with the terms "DIDMesh" Decentralized Identifier Method Namespace Specification and "DIDMesh" DID Method Namespace Specification.
- A compliant "DIDMesh" runtime library is a software library that, in part, implements the "DIDMesh" DID Document CRUD Abstract Interface in a way that complies with the "DIDMesh" DID Method Namespace Specification.
...
## 2. "DIDMesh" Identifier Method Name

The name string that shall identify the "DIDMesh" Identifier Method is: `mesh`.
...
## 3. "DIDMesh" Identifier Format

"DIDMesh" Identifiers **MUST** use the following format:
```
...
"DIDMesh" Identifier `id-string` values are encoded using the KERI `Base64` encoding method.
...
### Example 1. "DIDMesh" Interfaces Definition Identifier
```
...
The "DIDMesh" library implements CRUD interfaces for controlling the lifecycle of a "DIDMesh" Identifier and its associated DID Document that are compliant with this specification.
 
### 4.1 Create (Register)

To create a "DIDMesh" Identifier, a program invokes the `RegIdWithPublicKey` function from a compliant "DIDMesh" runtime library. 
The interface to register a "DIDMesh" Identifier and its associated public key is defined as follows:
```csharp
public bool RegIdWIthPublicKey(string didobjectid, byte[] publicKey); 
```
The calling program must include two parameters: the string value of the new "DIDMesh" Identifier to be registered and 
a cryptographic public key to act as the first management key. 
This function will return `True` if the "DIDMesh" Identifier had not been registered previously.

### 4.2 Read (Resolve)

"DIDMesh" Identifier's associated DID Document can be looked up by invoking the `GetDIDDocument` function from a compliant "DIDMesh" runtime library. 
...
The interface for resolving a "DIDMesh" Identifier and return its associated DID Document is defined as follows:
```csharp
...
Every public key in the array of `verificationMethod` can be used to authenticate the "DIDMesh" Controller.

Note: The list of supported public key signature schemes is listed in [Appendix A](#appendix-a-public-key-algorithm).

### Example 2. "DIDMesh" DID Document
...
To update the DID Document associated with a "DIDMesh" Identifier, two functions need to be invoked, 
```csharp
...
```
Note that `sender` param must be a currently-in-use public key of this "DIDMesh" Identifier.
If a public key is removed, then it **cannot** be added again.

### 4.4 Deactivate (Revoke)

To delete or deactivate a "DIDMesh", it suffices to remove all public keys from its associated 
DID Document. In this case, there is no public key that can be used to authenticate the "DIDMesh" Controller.

More importantly, deletion of a "DIDMesh" DID Document means that the associated "DIDMesh" Identifier cannot be reactivated again. 
...
used to develop DIDComm agents, services, and libraries that, in turn, leverage the "DIDMesh" Method Namespace Specification; as one example, whether the underlying VDR is configured to support concensus by PoA, PoS, or PoW.
...
The syntax and construction of a "DIDMesh" Identifier and its associated DID Document helps to ensure that no Personally Identifiable Information (PII) or other personal data is exposed by these constructs.

Implementers need to be aware of the privacy implications of the underlying tools and technologies 
used to develop DIDComm agents, services, and libraries that in turn, leverage the "DIDMesh" Method Namespace Specification. 
...
A version of the code for the "DIDMesh" Identifier Method reference implementation can be found in the following GitHub project: https://github.com/mwherman2000/TrustedDigitalWeb. 
This project is the definitive reference implementation of the "DIDMesh" Identifier Method and contains compliant reference implementations of apps, DIDComm agents, and services.
