# Trusted Digital Web "BlueToque Business Document" Decentralized Identifier Method Specification version 1.0.0
...
The "BlueToque Business Document" Decentralized Identifier Method specification 
("BlueToque Business Document" DID Method specification) 
...
The "BlueToque Business Document" DID Method space is based, in large part, on the OASIS Universal Business Language 2.2 (UBL) specification[41] that documents the 80+ most popular business document schemas (e.g. purchase orders, invoices, waybills, delivery confirmations, etc.).

This specification defines the following:
- "BlueToque Business Document" Identifier Syntax and Construction
- "BlueToque Business Document" DID Document CRUD Abstract Interface
...
that are compliant with the "BlueToque Business Document" DID Method specification.

### Out-of-Scope

The following topics are out-of-scope:
- Descriptions of any specific design, implementation, or deployment details for any particular software platform that might support the "BlueToque Business Document" DID Method.
...
- The term "BlueToque Business Document" Identifier is synonymous with the term "BlueToque Business Document" Decentralized Identifier. 
- The term "BlueToque Business Document" DID Document is synonymous with the term "BlueToque Business Document" Decentralized Identifier Document.
- The term "BlueToque Business Document" DID Method specification is synonymous with the terms "BlueToque Business Document" Decentralized Identifier Method specification and Trusted Digital Web "BlueToque Business Document" DID Method specification.
- A compliant Trusted Digital Web Runtime Library is a software library that, in part, implements the "BlueToque Business Document" DID Document CRUD Abstract Interface in a way that complies with the "BlueToque Business Document" DID Method specification.
...
## 2. Trusted Digital Web "BlueToque Business Document" Identifier Method Name

The name string that shall identify the "BlueToque Business Document" Identifier Method is: `bluetoquedoc`.
...
## 3. Trusted Digital Web "BlueToque Business Document" Identifier Format

There are two different types of "BlueToque Business Document" Identifiers on the Trusted Digital Web:
- "BlueToque Business Document" schema
- "BlueToque Business Document" documents

"BlueToque Business Document" Identifiers on the Trusted Digital Web **MUST** use the following format:
```
...
Trusted Digital Web "BlueToque Business Document" Identifier `id-string` values are encoded using the KERI `Base64` encoding method.
...
### Example 1. Trusted Digital Web "BlueToque Business Document" Schema Identifier
```
...
```
### Example 2. Trusted Digital Web "BlueToque Business Document" Document Identifier
```
...
"BlueToque Business Document" Identifiers and associated DID Documents on the Trusted Digital Web are managed by a compliant Trusted Digital Web Runtime Library. 
The Library implements CRUD interfaces for controlling the lifecycle of a "BlueToque Business Document" Identifier and its associated DID Document that are compliant with this specification.
 
### 4.1 Create (Register)

To create a "BlueToque Business Document" Identifier, a program invokes the `RegIdWithPublicKey` function from a compliant Trusted Digital Web Runtime Library. 
The interface to register a "BlueToque Business Document" Identifier and its associated public key is defined as follows:
```csharp
public bool RegIdWIthPublicKey(string didobjectid, byte[] publicKey); 
```
The calling program must include two parameters: the string value of the new "BlueToque Business Document" Identifier to be registered and 
a cryptographic public key to act as the first management key. 
This function will return `True` if the "BlueToque Business Document" Identifier had not been registered previously.

### 4.2 Read (Resolve)

Trusted Digital Web "BlueToque Business Document" Identifier's associated DID Document can be looked up by invoking the `GetDIDDocument` function from a compliant Trusted Digital Web Runtime Library. 
...
The interface for resolving a "BlueToque Business Document" Identifier and return its associated DID Document is defined as follows:
```csharp
...
Every public key in the array of `verificationMethod` can be used to authenticate the "BlueToque Business Document" Controller.

Note: The list of supported public key signature schemes is listed in [Appendix A](#appendix-a-public-key-algorithm).

### Example 2. Trusted Digital Web "BlueToque Business Document" DID Document
...
To update the DID Document associated with a "BlueToque Business Document" Identifier, two functions need to be invoked, 
```csharp
...
```
Note that `sender` param must be a currently-in-use public key of this "BlueToque Business Document" Identifier.
If a public key is removed, then it **cannot** be added again.

### 4.4 Deactivate (Revoke)

To delete or deactivate a "BlueToque Business Document", it suffices to remove all public keys from its associated 
DID Document. In this case, there is no public key that can be used to authenticate the "BlueToque Business Document" Controller.

More importantly, deletion of a Trusted Digital Web "BlueToque Business Document" DID Document means that the associated "BlueToque Business Document" Identifier cannot be reactivated again. 
...
that, in turn, leverage the "BlueToque Business Document" Method specification; as one example, whether the underlying VDR is configured to support concensus by PoA, PoS, or PoW 
...
The syntax and construction of a "BlueToque Business Document" Identifier and its associated DID Document helps to ensure that no Personally Identifiable Information (PII) or other personal data is exposed by these constructs.

Implementers need to be aware of the privacy implications of the underlying tools and technologies 
used to develop agents, services, and libraries for the Trusted Digital Web that, that in turn, leverage the "BlueToque Business Document" Method specification. 
...
The current version of the code for the "BlueToque Business Document" Identifier Method reference implementation can be found in the following GitHub project: https://github.com/mwherman2000/TrustedDigitalWeb. 
This project is the definitive reference implementation of the Trusted Digital Web "BlueToque Business Document" Identifier Method and contains compliant reference implementations of apps, agents, and services deployable on the Trusted Digital Web.
...
The Trusted Digital Web includes a Trusted Digital Web Runtime Library that is compliant with the "BlueToque Business Document" DID Method specification. "BlueToque Business Document" Identifiers and the "BlueToque Business Document" Identifer Method are two of the components and specifications that are foundational in the architecture and design of the Trusted Digital Web.
