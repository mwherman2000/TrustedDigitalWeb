# Trusted Digital Web (TDW) DID Nature DID Method Namespace Specification version 1.0.1 (Web 7.0 Sharded Registry)
...
This DID Nature DID Method Namespace Specification (DID Nature DID Method Namespace Specification) 
...
- DID Nature Identifier Syntax and Construction
- DID Nature DID Document CRUD Abstract Interface
...
that are compliant with the DID Nature DID Method Namespace Specification.

### Out-of-Scope

The following topics are out-of-scope:
- Descriptions of any specific design, implementation, deployment or other internal details for any particular software platform that might support the DID Nature DID Method.
...
- The term DID Nature Identifier is synonymous with the term DID Nature. 
- The term DID Nature DID Document is synonymous with the term DID Nature Document.
- The term DID Nature DID Method Namespace Specification is synonymous with the terms DID Nature DID Method Namespace Specification and Trusted Digital Web DID Nature DID Method Namespace Specification.
- A compliant Trusted Digital Web Runtime Library is a software library that, in part, implements the DID Nature DID Document CRUD Abstract Interface in a way that complies with the DID Nature DID Method Namespace Specification.
...
The authoriative source text for this specifcation can be found here: https://github.com/mwherman2000/TrustedDigitalWeb/blob/master/specifications/did-methods/did-nature-1-0-1.md.

## 2. Trusted Digital Web (TDW) DID Nature Identifier Method Name

The name strings that shall identify the primary DID Nature Identifier Method are:
...
## 3. Trusted Digital Web (TDW) DID Nature Identifier Format

DID Nature Identifiers on the Trusted Digital Web (TDW) **MUST** use the following format:
```
...
Trusted Digital Web DID Nature Identifier `id-string` values are encoded using the KERI `Base64` encoding method.
...
### TDW DID Nature DID Method Taxonomy

The TDW DID Nature DID Method Taxonomy is based on the Wikipedia Content Cateogies taxonomy: [Wikipedia:Contents/Categories](https://en.wikipedia.org/wiki/Wikipedia:Contents/Categories).

### Example 1. Trusted Digital Web (TDW) DID Nature Identifier
```
did:nature:BF5pxRJP6THrUtlDdhh07hJEDKrJxkcR9m5u1xs33bhp
```
...
DID Nature Identifiers and associated DID Documents on the Trusted Digital Web are managed by a compliant Trusted Digital Web Runtime Library. 
The Library implements CRUD interfaces for controlling the lifecycle of a DID Nature Identifier and its associated DID Document that are compliant with this specification.
 
### 4.1 Create (Register)

To create a DID Nature Identifier, a program invokes the `RegIdWithPublicKey` function from a compliant Trusted Digital Web Runtime Library. 
The interface to register a DID Nature Identifier and its associated public key is defined as follows:
```csharp
public bool RegIdWIthPublicKey(string didobjectid, byte[] publicKey); 
```
The calling program must include two parameters: the string value of the new DID Nature Identifier to be registered and 
a cryptographic public key to act as the first management key. 
This function will return `True` if the DID Nature Identifier had not been registered previously.

### 4.2 Read (Resolve)

Trusted Digital Web (TDW) DID Nature Identifier's associated DID Document can be looked up by invoking the `GetDIDDocument` function from a compliant Trusted Digital Web Runtime Library. 
...
The interface for resolving a DID Nature Identifier and return its associated DID Document is defined as follows:
```csharp
...
Every public key in the array of `verificationMethod` can be used to authenticate the DID Nature Controller.

Note: The list of supported public key signature schemes is listed in [Appendix A](#appendix-a-public-key-algorithm).

### Example 2. Trusted Digital Web (TDW) DID Nature DID Document

```json
{
    "id": "did:nature:BF5pxRJP6THrUtlDdhh07hJEDKrJxkcR9m5u1xs33bhp",
    "verificationMethod": [
        {
            "id": "did:nature:BF5pxRJP6THrUtlDdhh07hJEDKrJxkcR9m5u1xs33bhp#key-1",
            "type": "Ed25519VerificationKey2020",
            "controller": "did:nature:1234",
            "publicKeyMultibase": "zEY59y7px76e2yv5FMj9fYcjDsqk8yus6isWtkF69ZrHY"
        }
    ],
    "authentication": ["did:nature:BF5pxRJP6THrUtlDdhh07hJEDKrJxkcR9m5u1xs33bhp#key-1"],
    "assertionMethod": ["did:nature:BF5pxRJP6THrUtlDdhh07hJEDKrJxkcR9m5u1xs33bhp#key-1"],
    "service": [{
        "id":"#agent1",
...
To update the DID Document associated with a DID Nature Identifier, two functions need to be invoked, 
```csharp
...
```
Note that `sender` param must be a currently-in-use public key of this DID Nature Identifier.
If a public key is removed, then it **cannot** be added again.

### 4.4 Deactivate (Revoke)

To delete or deactivate a DID Nature, it suffices to remove all public keys from its associated 
DID Document. In this case, there is no public key that can be used to authenticate the DID Nature Controller.

More importantly, deletion of a Trusted Digital Web DID Nature DID Document means that the associated DID Nature Identifier cannot be reactivated again. 
...
that, in turn, leverage the DID Nature DID Method Namespace Specification; as one example, whether the underlying VDR is configured to support concensus by PoA, PoS, or PoW 
...
The syntax and construction of a DID Nature Identifier and its associated DID Document helps to ensure that no Personally Identifiable Information (PII) or other personal data is exposed by these constructs.

Implementers need to be aware of the privacy implications of the underlying tools and technologies 
used to develop agents, services, and libraries for the Trusted Digital Web that, that in turn, leverage the DID Nature DID Method Namespace Specification. 
...
The current version of the code for the DID Nature Identifier Method reference implementation can be found in the following GitHub project: https://github.com/mwherman2000/TrustedDigitalWeb. 
This project is the definitive reference implementation of the Trusted Digital Web (TDW) DID Nature Identifier Method and contains compliant reference implementations of apps, agents, and services deployable on the Trusted Digital Web.
...
The Trusted Digital Web includes a Trusted Digital Web Runtime Library that is compliant with the DID Nature DID Method Namespace Specification. DID Nature Identifiers and the DID Nature Identifer Method are two of the components and specifications that are foundational in the architecture and design of the Trusted Digital Web.
