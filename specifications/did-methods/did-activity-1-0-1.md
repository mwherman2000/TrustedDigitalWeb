# Trusted Digital Web (TDW) DID Activity DID Method Namespace Specification version 1.0.1 (Web 7.0 Sharded Registry)
...
This DID Activity DID Method Namespace Specification (DID Activity DID Method Namespace Specification) 
...
- DID Activity Identifier Syntax and Construction
- DID Activity DID Document CRUD Abstract Interface
...
that are compliant with the DID Activity DID Method Namespace Specification.

### Out-of-Scope

The following topics are out-of-scope:
- Descriptions of any specific design, implementation, deployment or other internal details for any particular software platform that might support the DID Activity DID Method.
...
- The term DID Activity Identifier is synonymous with the term DID Activity. 
- The term DID Activity DID Document is synonymous with the term DID Activity Document.
- The term DID Activity DID Method Namespace Specification is synonymous with the terms DID Activity DID Method Namespace Specification and Trusted Digital Web DID Activity DID Method Namespace Specification.
- A compliant Trusted Digital Web Runtime Library is a software library that, in part, implements the DID Activity DID Document CRUD Abstract Interface in a way that complies with the DID Activity DID Method Namespace Specification.
...
The authoriative source text for this specifcation can be found here: https://github.com/mwherman2000/TrustedDigitalWeb/blob/master/specifications/did-methods/did-activity-1-0-1.md.

## 2. Trusted Digital Web (TDW) DID Activity Identifier Method Name

The name strings that shall identify the primary DID Activity Identifier Method are:
...
## 3. Trusted Digital Web (TDW) DID Activity Identifier Format

DID Activity Identifiers on the Trusted Digital Web (TDW) **MUST** use the following format:
```
...
Trusted Digital Web DID Activity Identifier `id-string` values are encoded using the KERI `Base64` encoding method.
...
### TDW DID Activity DID Method Taxonomy

The TDW DID Activity DID Method Taxonomy is based on the Wikipedia Content Cateogies taxonomy: [Wikipedia:Contents/Categories](https://en.wikipedia.org/wiki/Wikipedia:Contents/Categories).

### Example 1. Trusted Digital Web (TDW) DID Activity Identifier
```
did:activity:BF5pxRJP6THrUtlDdhh07hJEDKrJxkcR9m5u1xs33bhp
```
...
DID Activity Identifiers and associated DID Documents on the Trusted Digital Web are managed by a compliant Trusted Digital Web Runtime Library. 
The Library implements CRUD interfaces for controlling the lifecycle of a DID Activity Identifier and its associated DID Document that are compliant with this specification.
 
### 4.1 Create (Register)

To create a DID Activity Identifier, a program invokes the `RegIdWithPublicKey` function from a compliant Trusted Digital Web Runtime Library. 
The interface to register a DID Activity Identifier and its associated public key is defined as follows:
```csharp
public bool RegIdWIthPublicKey(string didobjectid, byte[] publicKey); 
```
The calling program must include two parameters: the string value of the new DID Activity Identifier to be registered and 
a cryptographic public key to act as the first management key. 
This function will return `True` if the DID Activity Identifier had not been registered previously.

### 4.2 Read (Resolve)

Trusted Digital Web (TDW) DID Activity Identifier's associated DID Document can be looked up by invoking the `GetDIDDocument` function from a compliant Trusted Digital Web Runtime Library. 
...
The interface for resolving a DID Activity Identifier and return its associated DID Document is defined as follows:
```csharp
...
Every public key in the array of `verificationMethod` can be used to authenticate the DID Activity Controller.

Note: The list of supported public key signature schemes is listed in [Appendix A](#appendix-a-public-key-algorithm).

### Example 2. Trusted Digital Web (TDW) DID Activity DID Document

```json
{
    "id": "did:activity:BF5pxRJP6THrUtlDdhh07hJEDKrJxkcR9m5u1xs33bhp",
    "verificationMethod": [
        {
            "id": "did:activity:BF5pxRJP6THrUtlDdhh07hJEDKrJxkcR9m5u1xs33bhp#key-1",
            "type": "Ed25519VerificationKey2020",
            "controller": "did:activity:1234",
            "publicKeyMultibase": "zEY59y7px76e2yv5FMj9fYcjDsqk8yus6isWtkF69ZrHY"
        }
    ],
    "authentication": ["did:activity:BF5pxRJP6THrUtlDdhh07hJEDKrJxkcR9m5u1xs33bhp#key-1"],
    "assertionMethod": ["did:activity:BF5pxRJP6THrUtlDdhh07hJEDKrJxkcR9m5u1xs33bhp#key-1"],
    "service": [{
        "id":"#agent1",
...
To update the DID Document associated with a DID Activity Identifier, two functions need to be invoked, 
```csharp
...
```
Note that `sender` param must be a currently-in-use public key of this DID Activity Identifier.
If a public key is removed, then it **cannot** be added again.

### 4.4 Deactivate (Revoke)

To delete or deactivate a DID Activity, it suffices to remove all public keys from its associated 
DID Document. In this case, there is no public key that can be used to authenticate the DID Activity Controller.

More importantly, deletion of a Trusted Digital Web DID Activity DID Document means that the associated DID Activity Identifier cannot be reactivated again. 
...
that, in turn, leverage the DID Activity DID Method Namespace Specification; as one example, whether the underlying VDR is configured to support concensus by PoA, PoS, or PoW 
...
The syntax and construction of a DID Activity Identifier and its associated DID Document helps to ensure that no Personally Identifiable Information (PII) or other personal data is exposed by these constructs.

Implementers need to be aware of the privacy implications of the underlying tools and technologies 
used to develop agents, services, and libraries for the Trusted Digital Web that, that in turn, leverage the DID Activity DID Method Namespace Specification. 
...
The current version of the code for the DID Activity Identifier Method reference implementation can be found in the following GitHub project: https://github.com/mwherman2000/TrustedDigitalWeb. 
This project is the definitive reference implementation of the Trusted Digital Web (TDW) DID Activity Identifier Method and contains compliant reference implementations of apps, agents, and services deployable on the Trusted Digital Web.
...
The Trusted Digital Web includes a Trusted Digital Web Runtime Library that is compliant with the DID Activity DID Method Namespace Specification. DID Activity Identifiers and the DID Activity Identifer Method are two of the components and specifications that are foundational in the architecture and design of the Trusted Digital Web.
