...
namespace TDW.TDACommon
{
    public unsafe class longArray_4 : IEnumerable<long>
    {
        
        private static readonly int SizeDim0 = 4;
        ...
        internal byte* m_ptr;
        internal long m_cellId;
        internal longArray_4(byte* _CellPtr)
        {
            this.m_ptr = _CellPtr;
        }
        /// <summary>
        ...
        public unsafe byte[] ToByteArray()
        {
            byte[] ret = new byte[Length * 8];
            fixed (byte* ptr = ret)
            {
                Memory.Copy(m_ptr, ptr, Length * 8);
            }
            return ret;
        }
        ...
        internal unsafe struct _iterator
        {
            ...
            byte* endPtr;
            longArray_4 target;
            internal _iterator(longArray_4 target)
            {
                targetPtr   = target.m_ptr;
                endPtr      = target.m_ptr + target.Length * 8;
                this.target = target;
            }
            internal bool good()
            {
        ...
        public unsafe void Clear(int index, int length)
        {
            ...
            Memory.memset(m_ptr + index* 8, 0, (ulong)(length * 8));
        }
        public unsafe static implicit operator long[    ]
            (longArray_4 accessor)
        {
            long[    ] ret 
                = new long[  4  ];
            ...
            fixed (long* p = ret)
            {
                Memory.Copy(accessor.m_ptr, p, 32);
            }
            
            return ret;
        }
        
        public unsafe static implicit operator longArray_4(long[] field)
        {
            byte* targetPtr = null;
            targetPtr += 32;
            ...
            targetPtr = tmpcellptr;
                ...
                if(field.GetLength(0) != 4) throw new IndexOutOfRangeException("The assigned array'storage dimension mismatch.");
               fixed(long* storedPtr_1 = field)
                   Memory.memcpy(targetPtr, storedPtr_1, (ulong)(32));
            } else {
...
longArray_4 ret;
            ...
            ret = new longArray_4(tmpcellptr);
            
            return ret;
        }
        
        public static bool operator == (longArray_4 a, longArray_4 b)
        {
            if (ReferenceEquals(a, b))
              return true;
            if (ReferenceEquals(a, null) || ReferenceEquals(b, null))
              return false;
            if (a.m_ptr == b.m_ptr) return true;
            return Memory.Compare(a.m_ptr, b.m_ptr, a.Length * 8);
        }
        public static bool operator != (longArray_4 a,longArray_4 b)
        {
            return !(a == b);
