...
namespace TDW.TRAServer
{
    ...
    public partial struct TRATimestampEnvelope
    {
        ...
        ///</summary>
        public TRATimestampEnvelope(TRATimestampContent content = default(TRATimestampContent),TRACredential_Label label = default(TRACredential_Label))
        {
...

{
    ...
    if(field.content.context!= null)
    {
        ...
        }else
        {
            ...
            targetPtr += 8;
targetPtr += 8;
        if(field.content.claims.Value.timestamp!= null)
        {
            int strlen_4 = field.content.claims.Value.timestamp.Length * 2;
            targetPtr += strlen_4+sizeof(int);
        }else
        {
            targetPtr += sizeof(int);
...
*(int*)storedPtr_3 = (int)(targetPtr - storedPtr_3 - 4);
            ...
            {
            *(long*)targetPtr = field.content.claims.Value.ticks;
            targetPtr += 8;

        {
            *(long*)targetPtr = field.content.claims.Value.datetime.ToBinary();
            targetPtr += 8;
        }
        ...
        if(field.content.claims.Value.timestamp!= null)
        {
            ...
            *(int*)targetPtr = strlen_4;
            targetPtr += sizeof(int);
            fixed(char* pstr_4 = field.content.claims.Value.timestamp)
            {
                Memory.Copy(pstr_4, targetPtr, strlen_4);
                targetPtr += strlen_4;
            }
